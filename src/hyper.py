...
def print_header(x: int, y: int):
    if latex:
        latex_converter.latex_header(x,y)
    else:
        print(f"|X| = {x}")
        print(f"|Y| = {y}")
...
def print_prior(prior: np.ndarray):
    if latex:
        latex_converter.latex_array(prior,"\\pi")
    else:
        print(f"Prior distribution:\n{prior}\n")
    
def print_channel(cmatrix: np.ndarray, clabel: str):
    if latex:
        latex_converter.to_latex(f"Channel {clabel} matrix", clabel, cmatrix)
    else:
        print(f"Channel matrix:\n{cmatrix}\n")

def print_joint(jmatrix: np.ndarray):
    if latex:
        latex_converter.to_latex(f"Joint matrix", "J", jmatrix)
    else:
        print(f"Joint matrix:\n{jmatrix}\n")

def print_yprob(py: np.ndarray):
    if latex:
        latex_converter.latex_array(py,"p_\\mathsf{Y}")
    else:
        print(f"P[Y]:\n{py}\n")

def print_posterior(pmatrix: np.ndarray):
    if latex:
        latex_converter.to_latex(f"Posterior matrix", "", pmatrix, type="posterior")
    else:
        print(f"Posterior distribuition(xi,Pxi|Y):\n{pmatrix}\n")

def print_hyper(hmatrix: np.ndarray, pyn: np.ndarray, clabel: str):
    if latex:
        latex_converter.to_latex(f"Hyper Distribution", clabel, hmatrix, type="hyper", Y=pyn)
    else:
        print(f"PHyper[Y]:\n{pyn}\n")
